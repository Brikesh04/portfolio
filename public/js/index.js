; (async function () {
    const getSettings = () => {
        return new Promise((resolve) => {
            if (window.__SETTINGS) {
                resolve(window.__SETTINGS);
            } else {
                window.addEventListener('settingsready', (e) => {
                    resolve(e.detail);
                }, { once: true });
            }
        });
    ...
    // Dynamically update profile links, emails, and names
    document.querySelectorAll('a[href^="mailto:"]').forEach(el => {
        el.href = "mailto:" + settings.email;
        if (el.textContent.includes('@')) {
            el.textContent = settings.email;
        }
        if (el.getAttribute('data-chr-footer')) {
            el.setAttribute('data-chr-footer', settings.email);
        }
        if (el.getAttribute('data-chr')) {
            el.setAttribute('data-chr', settings.email);
        }
    });
    ...
    try {
        ...
        console.warn('Backend offline, running with offline project fallbacks.', err);
    }
    ...
    const pBrikesh = document.getElementById('preloader-luke');
    const pVikin = document.getElementById('preloader-baffait');
    ...
    const lukeChars = splitIntoChars(pBrikesh);
    const baffaitChars = splitIntoChars(pVikin);
    const allChars = [...lukeChars, ...baffaitChars];
    const allRevealEls = [...logoChar, ...lukeChars, ...baffaitChars];


    function getCharGap() {
        return parseFloat(getComputedStyle(pVikin).fontSize) * 0.55;
    }

    function layoutNames() {
        const fs = parseFloat(getComputedStyle(pVikin).fontSize);
        if (!fs) return;
        const baselineOffset = -0.06;

        const lukeLeft = pBrikesh.offsetLeft;
        const lukeWidth = pBrikesh.offsetWidth;
        const gapPx = fs * 0.55;

        const baffaitLeftPx = lukeLeft + lukeWidth + gapPx;
        pVikin.style.left = (baffaitLeftPx / fs) + 'em';
        pVikin.style.top = baselineOffset + 'em';

        const dotLeftPx = baffaitLeftPx + pVikin.offsetWidth;
    ...
    gsap.set([pContent, tPanelRed, tPanelDark], {
        willChange: 'transform'
    });

    function getTotalWidth() {
        return pLogo.offsetWidth + pBrikesh.offsetWidth + getCharGap() + pVikin.offsetWidth + pDot.offsetWidth;
    }
    ...
    master
        ...
        .add(() => {
            layoutNames();
            gsap.set(pContent, {
                x: -(getTotalWidth() / 2 - pLogo.offsetWidth / 2)
            });
            gsap.set(pBrikesh, {
                x: 0
            });
        ...
        .add(() => {
            ...
            const applyFinalState = () => {
                ...
                [pLogo, pBrikesh, pVikin, pDot].forEach(el => {
    ...
    const chrHoverTl = gsap.timeline({
        paused: true
    });
    ...
    async function setupScrollReveal() {
        ...
        [pContent, pLogo, pBrikesh, pVikin, pDot].forEach(el => gsap.killTweensOf(el));
        ...
        async function loadFirstBatch() {
            ...
            if (!first) {
                console.warn('Frame 1 failed to load, falling back to static hero sequence frame 1.');
                ...
                fallbackImg.src = '/assets/images/hero sequence/0001.jpg';
                const fallback = await loadPromise;
                if (fallback) {
                    frames[0] = fallback;
        ...
        if (totalFrames === 0) {
            console.warn('No reveal frames loaded. Skipping canvas sequence, but continuing scroll setup.');
        ...
        scrollTl.fromTo(revealWrap, {
            opacity: 0
        }, {
            opacity: 1,
            duration: 0.01
        }, 0.3);
        scrollTl.fromTo(revealSeq, {
            scale: 0
        }, {
            scale: 1,
            duration: 0.7,
        ...
        exitTl.to(revealWrap, {
            y: '-50vh',
            ease: 'none',
            duration: 1
        }, 0);
        exitTl.to(revealOverlay, {
            opacity: 0.7,
            ease: 'none',
            duration: 0.66
        }, 0);
        ...
        const phraseExitTl = gsap.timeline({
            paused: true
        });
    ...
    function setupAboutSection() {
        ...
        if (isMobile) {
            [...aboutText.querySelectorAll('.word'), ...aboutVersion.querySelectorAll('.word')].forEach(w => {
                w.style.filter = 'none';
            });
        }
    ...
    function setupProjectsSection() {
        ...
        if (dbProjects && dbProjects.length > 0) {
            dbProjects.forEach(proj => {
                PROJECTS[proj.id] = {
                    desc: isEn ? proj.desc_en : proj.desc_fr,
                    category: isEn ? proj.category_en : proj.category_fr,
                    year: proj.year,
                    tags: proj.tags,
                    images: proj.images
                };
            ...
            // Dynamically render homepage projects list
            ...
            if (listContainer) {
                ...
                dbProjects.forEach(proj => {
                    ...
                    item.setAttribute('data-img', proj.img);
                    item.setAttribute('data-date', proj.date);
                    item.textContent = proj.title;
                    listContainer.appendChild(item);
                });
            }

            // Dynamically render circle gallery images
            const galleryContainer = document.getElementById('circle-gallery-pin');
            if (galleryContainer) {
                ...
                if (phrase) {
                    galleryContainer.appendChild(phrase);
                }
            }
        } else {
            PROJECTS = {
        ...
        function closeProject() {
            ...
            tl.add(function () {
                ...
                gsap.set([detailTitleWrap, detailDesc, detailTags, detailBack, detailGalleryWrap], {
                    opacity: 0
                });
                gsap.set(flyingTitle, {
                    opacity: 0
    ...
    const PAGE_LINK_ROUTES = {
        work: {
            label: 'Work',
            sessionKey: 'work-transition',
            href: 'works/'
        },
        info: {
            label: 'Info',
            sessionKey: 'info-transition',
            href: 'info/'
        },
        contact: {
            label: 'Contact',
